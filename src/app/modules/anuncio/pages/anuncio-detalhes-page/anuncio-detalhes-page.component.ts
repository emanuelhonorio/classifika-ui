...
export class AnuncioDetalhesPageComponent implements OnInit {
  ...
  constructor(private activatedRoute: ActivatedRoute, private anuncioService: AnuncioService, private localizacaoService: LocalizacaoService) { }

  ngOnInit() {
    let idAnuncio = Number.parseInt(this.activatedRoute.snapshot.paramMap.get('id'));
    this.initAnuncio(idAnuncio);
  }

  initAnuncio(idAnuncio: number) {
    this.anuncioService.buscarPorId(idAnuncio).subscribe((anuncio: Anuncio) => {
  ...
  initLocalizacao(cep: string) {
    this.localizacaoService.consultarCep(cep).subscribe((data) => {
  ...
  this.galleryImages = [];
    for (let foto of fotos) {
      this.galleryImages.push({ small: foto.url, medium: foto.url, big: foto.url });
    }
  }
